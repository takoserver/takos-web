...
export function SelectedServer(
    ...
    const [serverDetails, setServerDetails] = createSignal<{domain: string, name: string, explain: string, iconUrl: string}[]>([]);
    ...
    const fetchServerDetails = async (domain: string) => {
        ...
        try {
            ...
            // サーバー説明を取得
            const explainResp = await fetch(`https://${domain}/api/v2/server/explain`);
            const explainData = await explainResp.json();
            
            // アイコンURLを設定
            const iconUrl = `https://${domain}/api/v2/server/icon`;
            console.log(`Fetched details for ${domain}:`, nameData, explainData, iconUrl);
        ...
        <div class="flex flex-col items-center min-h-screen w-screen bg-black text-white py-6 px-4 pb-12">
            ...
            <div class="w-full max-w-2xl mx-auto">
                ...
                <div class="text-center mb-8 mt-8">
                    ...
                    <p class="text-gray-400 text-base mb-6">サーバーを選択またはドメインで追加</p>
                ...
                <div class="w-full mb-10">
                    {servers().length === 0 ? (
                        <div class="text-center py-8 bg-gray-900 rounded-md border border-gray-800">
                            <div class="text-gray-400 text-lg mb-2">サーバーがまだ追加されていません</div>
                            <p class="text-gray-600">「サーバーを追加」ボタンをクリックして始めましょう</p>
                        </div>
                    ) : (
                        <div class="flex flex-col space-y-3 w-full">
                            <For each={servers()}>
                                {(domain) => {
                                    ...
                                    const details = () => serverDetails().find(item => item.domain === domain);
                                    return (
                                        <div 
                                        ...
                                        >
                                            ...
                                            <div class="p-3 flex-grow">
                                                <h3 class="text-md font-medium truncate">{details()?.name || domain}</h3>
                                                <p class="text-gray-500 text-sm mb-1 truncate">{domain}</p>
                                                {details()?.explain && (
                                                    <p class="text-gray-400 text-xs line-clamp-2">{details()!.explain}</p>
                                                )}
                                            </div>
            ...
            {showModal() && (
                <div class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
                    <div class="bg-gray-900 rounded-md w-full max-w-md p-6 border border-gray-800">
                        ...
                        <div class="mb-5">
                            <label class="block text-gray-400 mb-2 text-sm" for="serverDomain">サーバードメイン</label>
                            <div class="relative">
                                <input
                                    id="serverDomain"
                                    type="text"
